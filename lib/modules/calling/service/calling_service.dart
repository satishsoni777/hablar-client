...
class CallingService extends HttpManager implements CallingApi {
  Future<Response> startMeeting() async {
    ...
    final response = await sendRequest(HttpMethod.POST,
        endPoint: Endpoints.startMeeting, request: body);
  ...
  Future<RoomsResponse> joinRandomRoom() async {
    ...
    try {
      final response = await sendRequest(HttpMethod.POST,
          endPoint: Endpoints.joinRandomRoom, request: req);
      if (response.isSuccesFull()) {
        res = RoomsResponse.fromJson(jsonDecode(response.data));
        await DI
            .inject<SharedStorage>()
            .setStringPreference(StorageKey.roomId, res.data?.roomId ?? '');
      }
    } on DioError catch (e) {
      throw e;
    }
  ...
  Future<dynamic> leaveRoom() async {
    final userId = await Utils.userId();
    final roomId =
        await DI.inject<SharedStorage>().getStringPreference(StorageKey.roomId);
    var response;
    final req = {"userId": userId};
    final query = {"roomId": roomId};
    try {
      response = await sendRequest(HttpMethod.POST,
          endPoint: Endpoints.leaveRoom, queryParameters: query, request: req);
    } on DioError catch (_) {
      throw _;
    }
    return response;
